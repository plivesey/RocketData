...
class SharedCollectionManager {
     ...
     */
    func updateProvider(provider: SharedCollection, cacheKey: String?, previousCacheKey: String?) {
        ...
        if let cacheKey = cacheKey {
            ...
            if var weakArray = providers[cacheKey] {
                if !weakArray.contains({ element in element === provider }) {
     ...
     */
    func siblingProvidersForProvider(provider: SharedCollection, cacheKey: String) -> [SharedCollection] {
        if var weakArray = providers[cacheKey] {
     ...
     */
    func dataFromProviders<T>(cacheKey cacheKey: String) -> DataHolder<[T]>? {
     ...
     */
    func dataFromProviders(providers: [SharedCollection]) -> DataHolder<[Any]>? {
        let maxElement = providers.maxElement { first, second in
     ...
     */
    private static func genericArrayFromArray<T>(array: [Any]) -> [T]? {
...
protocol SharedCollection: class {
     ...
     */
    func setAnyData(data: [Any], context: Any?)
    /**
     ...
     */
    func insertAny(data: [Any], at index: Int, context: Any?)
    /**
     ...
     It should however listen on any new elements.
     */
    func updateAny(element: Any, at index: Int, context: Any?)
    /**
     This should remove an element from the collection.
     It should NOT call update on the consistency manager or try to save to the cache.
     The original collection will already do this and we don't want to make multiple calls.
     */
    func removeAnyAtIndex(index: Int, context: Any?)
}
...
extension CollectionDataProvider: SharedCollection {
    ...
    func setAnyData(newData: [Any], context: Any?) {
    ...
    func insertAny(newData: [Any], at index: Int, context: Any?) {
        guard let newData: [T] = SharedCollectionManager.genericArrayFromArray(newData) else {
            return
        }
        ...
        if newData.count > 0 {
            ...
            if !paused {
                ...
                // Should return [1, 2]
                let changes = newData.enumerate().map { (elementIndex, _) in
    ...
    func updateDelegatesWithChange(change: CollectionChange, context: Any?) {
     ...
     */
    static func dataEqual(lhs: [T], rhs: [T]) -> Bool {
